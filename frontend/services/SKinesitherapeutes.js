const kinesitherapeutes = [
  {
    id: 1, name: "Cabinet Kinésithérapie Santé", speciality: "Rééducation Fonctionnelle", city: "Casablanca",
    rating: 4.8, verified: true, available: true, price: 200, category: "rééducation fonctionnelle",
    availability: "today", avatar: "CK",
  ...
  {
    id: 2, name: "Centre Kiné Pro", speciality: "Massage Thérapeutique", city: "Rabat",
    rating: 4.7, verified: true, available: true, price: 180, category: "massage thérapeutique",
    availability: "today", avatar: "CP",
    days: ["Aujourd'hui","Demain","Autre"],
    times: [
  ...
  {
    id: 3, name: "Institut Kinésithérapie Moderne", speciality: "Sport et Performance", city: "Marrakech",
    rating: 4.9, verified: true, available: true, price: 250, category: "sport et performance",
    availability: "week", avatar: "IK",
    days: ["Aujourd'hui","Demain","Autre"],
    times: [
  ...
  {
    id: 4, name: "Cabinet Kiné Bien-être", speciality: "Douleurs Chroniques", city: "Fès",
    rating: 4.6, verified: true, available: true, price: 150, category: "douleurs chroniques",
    availability: "today", avatar: "KB",
    days: ["Aujourd'hui","Demain","Autre"],
    times: [
...
// ===== RENDER KINESITHERAPEUTES =====
function renderKinesitherapeutes(data){
  const container = document.getElementById("kinesitherapeutes-list");
  if (!container) return;
  
  container.innerHTML = "";

  data.forEach(kinesitherapeute=>{
    const article=document.createElement("article");
    article.className="doctor-card";
    article.dataset.cat=kinesitherapeute.category;
    article.dataset.availability=kinesitherapeute.availability;

    article.innerHTML=`
      <div class="doctor-main">
        <div class="doctor-avatar">${kinesitherapeute.avatar}</div>
        <div class="doctor-info">
          <h3>${kinesitherapeute.name}</h3>
          <p>${kinesitherapeute.speciality} · ${kinesitherapeute.city}</p>
          <div class="doctor-meta">
            <span class="rating">⭐ ${kinesitherapeute.rating}</span>
            ${kinesitherapeute.verified?'<span class="badge verified">✔ Vérifié</span>':''}
            <span class="badge ${kinesitherapeute.available?"available":"unavailable"}">${kinesitherapeute.available?"Disponible":"Indisponible"}</span>
          </div>
        </div>
      ...
      <div class="doctor-actions">
        <div class="days">
          ${kinesitherapeute.days.map((d,i)=>`<button class="day ${i===0?'active':''}"><span>${d}</span></button>`).join("")}
        </div>
        <div class="times-wrapper">
          <button class="nav prev" aria-label="Précédent">‹</button>
          <div class="times">
            ${kinesitherapeute.times.map((page,i)=>`<div class="times-page ${i===0?'active':''}">${page.slots.map(s=>`<button class="time ${s.status}" ${s.status === 'reserved' ? 'disabled' : ''}>${s.time}</button>`).join("")}</div>`).join("")}
          </div>
      ...
      <div class="doctor-footer">
        <span class="price">Séance : <strong>${kinesitherapeute.price} DH</strong></span>
...
function attachEvents(){
  ...
  document.querySelectorAll(".nav").forEach(btn=>{
    btn.addEventListener("click",()=>{
      const kinesitherapeuteCard = btn.closest(".doctor-card");
      const wrapper = kinesitherapeuteCard.querySelector(".times");
  ...
  document.querySelectorAll(".reserve-btn").forEach(btn=>{
    btn.addEventListener("click",()=>{
      ...
      const kinesitherapeuteName=card.querySelector("h3").textContent;
...
// ===== INIT =====
renderKinesitherapeutes(kinesitherapeutes);
